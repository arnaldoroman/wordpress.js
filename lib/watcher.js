...
Watcher.prototype.loadPost = function (id, callback) {
    var self = this;
    if (!this.loaded) {
    ...
    this.blog.loadPost(id, this.metadata, {
        meta_keys: this.blog.options.postmeta_keys
    }, function (err, post) {
        if (err || !post) {
            return callback(err);
        }
        self.blog.loadTermRelationships([ post.id ], function (err, relationships) {
            if (err) return callback(err);
            self.blog.addPostTerms([ post ], self.metadata.terms, relationships);
            callback(null, post);
        });
    });
};
