...
Blog.prototype.loadPosts = function (options, callback) {
    ...
    */}, true);
    ...
    this.connection.iter(sql, args, function (row) {
        var post = self.buildPostInstance(row);
        if (options.meta_keys) {
            options.meta_keys.forEach(function (key, i) {
                post['meta_' + key] = row[8 + i];
            });
        }
        posts.push(post);
    }, function (err) {
...
/**
 * Build a post instance.
 *
 * @param {Array} row
 * @return {Post} post
 */

Blog.prototype.buildPostInstance = function (row) {
    var post = new Post();
    post.id = row[0];
    post.title = row[1];
    post.content = row[2];
    post.excerpt = row[3];
    post.date = new Date(row[4]);
    post.modified = new Date(row[5]);
    ...
    post.slug = '/' + year + '/' + month + '/' + (row[6] || utils.slug(row[1]));
